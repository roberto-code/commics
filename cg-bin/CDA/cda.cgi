...
my $statement = "library(candisc)\n";
$statement .= "input.data<-data.frame(read.table(\"".WORK_DIR."/cda_$file2open\"))\n";
$statement .= "input.mod <- lm(cbind($categories) ~ Environment, data=input.data)\n";
$statement .= "input.can <- candisc(input.mod, data=input.data)\n";
$statement .= "d.colors<-colors()\n";
my $vector = join(",", @rPchs);
$statement .= "pch <-c($vector)\n";
