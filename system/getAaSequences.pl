...
my $updateh = $dbh->prepare('UPDATE sequences SET SEQAA = ? WHERE METAID = ? AND GENEID = ?')
                or die "Couldn't prepare statement: " . $dbh->errstr;

my $inserth = $dbh->prepare('INSERT INTO sequences(SEQAA, METAID, GENEID) VALUES (?,?,?)')
                or die "Couldn't prepare statement: " . $dbh->errstr;
...
foreach $id ( @raw_data )
{
	...
	if( $id  !~ m/^#/ )
	{
		...
		{
			print "Processing $id\n";
			my $tmpfile = "./tmp/sequences/aasequences$id";
			if ( ! -e $tmpfile )
			{
				my $fileSize = 1;
		  		my $fileSizeServer = 0;
				...
				do
				{
					...
					my $url = "http://img.jgi.doe.gov/cgi-bin/m/main.cgi?section=TaxonDetail&downloadTaxonFaaFile=1&taxon_oid=".$id."&_noHeader=1";
				...
				{ 
					print "ERROR: The Aa sequences file $id wasn't correctly downloaded\n";
					unlink( $tmpfile );
				}
				else
				{ print "Done\n";}
